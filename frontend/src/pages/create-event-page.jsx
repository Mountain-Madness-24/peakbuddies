...
export const CreateEventPage = () => {
  ...
  const addLocation = () => {
    if (tempLocation && locationName) {
      setLocations([
        ...locations,
        { name: locationName, latLong: tempLocation },
      ]);
      setLocationName("");
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const { nameOfEvent, description, startDate, endDate } =
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ul>
          {locations.map((location, index) => (
            <li key={index}>
              {location.name}: Lat {location.latLong.lat}, Lng{" "}
